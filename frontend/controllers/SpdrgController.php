...
class SpdrgController extends \yii\web\Controller {
...
GROUP BY sl.GYEAR";
    ...
    public function actionPci() {
        ...
        $sql = "SELECT COUNT(SERVKEY) as n,concat(FUNDCODE,' ',FUNDNAME) as FUND 
FROM rs_pci as sl WHERE sl.DATEDSC_MONTH BETWEEN '$dch1' AND '$dch2' 
GROUP BY FUNDCODE";
        $rs = \Yii::$app->db->createCommand("$sql")->queryAll();
        foreach ($rs as $d) {
...
FROM rs_pci as sl WHERE sl.DATEDSC_MONTH BETWEEN '$dch1' AND '$dch2' AND sl.HMAINREGIONCODE=04  AND sl.FUNDCODE in ('IPAEC','IPINRGC')
...
GROUP BY HMAIN2";
    ...
    public function actionInregionpci() {
...
,COUNT(DISTINCT sl.SERVKEY) as N,SUM(sl.NETGLOBPAIDAMT) as NET FROM rs_pci as sl 
...
,COUNT(DISTINCT sl.SERVKEY) as N FROM rs_pci as sl 
WHERE sl.DATEDSC_MONTH BETWEEN '$dch1' AND '$dch2'
AND sl.REGIONCODE=04  
GROUP BY HCODE ORDER BY COUNT(DISTINCT sl.SERVKEY) DESC limit 10")->queryAll();
        ...
        $m_list = MList::find()->all();
        //$fundlist = Fundlist::find()->all();
        return $this->render('inregionpci', [
                    'n' => $n,'na' => $na,
                    'y' => $y,
                    'm_list' => $m_list,
                    'm1' => $m1,
     ...
     public function actionHmainpci() {
